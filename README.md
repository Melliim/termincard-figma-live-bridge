...
Figma plugin
→ local bridge server
→ browser-demo/design-tokens.css + design-tokens.json
→ browser preview
...
The Figma status buttons set the selected card to one of the three defined component states.

The status itself is not synced into the web app, because in a real application the appointment status would come from application/backend data.
...
When a generated TerminCard is selected in Figma, changing a token pair in the plugin immediately previews the active status token on that selected Figma card.

The actual codebase changes only after pressing Sync token pairs to Web Demo, which writes:
...
5. If Figma cannot reach the bridge, re-import figma-plugin/manifest.json.
